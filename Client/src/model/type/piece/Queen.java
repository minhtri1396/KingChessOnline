...
{
    private Queen(EPieces type, EPositions position, EPieceColors color, Board board, String imagePath) {
    ...
    public static List<Piece> make(EPieceColors color, Board board) {
        if (color == EPieceColors.BLACK) {
            return Arrays.asList(new Piece[] {
                new Queen(EPieces.QUEEN, EPositions.D8, color, board, "/pieces/black_queen.png")
            });
        }
        
        return Arrays.asList(new Piece[] {
            new Queen(EPieces.QUEEN, EPositions.D1, color, board, "/pieces/white_queen.png")
        });
    }
